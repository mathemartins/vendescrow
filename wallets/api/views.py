...
from wallets.models import EthereumWallet, TetherUSDWallet, BitcoinWallet
...
class BitcoinAddressDetailView(RetrieveAPIView):
    ...
    def get(self, request, **kwargs):
        try:
            ...
            balance = requests.get(
                'https://api.blockcypher.com/v1/btc/main/addrs/{address}/balance?token=79d9c1fb002c4543a0befb9e83d81a5c'.format(address=btc_account['address']))
            jsonBal = balance.content.decode('utf-8').replace("'", '"')
            jsonBal = json.loads(jsonBal)
            balance_in_btc = float(jsonBal.get('balance')) / 100000000
...
class TransferEthereum(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response(
            {'message': "transaction successful", "tx_": web3.toHex(tx_hash), "success": bool(web3.toHex(tx_hash))},
